...
;var CapitalDetails = {
	queryCapitalList: ()=>{	//押金明细
		...
		$.ajax({
			...
			data:{
				token:token,
				queryType: 3,	//1.押金明细  2.购买明细  3.资金明细
				pageNumber: CapitalDetails.z,				//分页的数目，如：1,2,3页
				pageSize: CapitalDetails.itemsPerLoad		//每一页的条数，如：1有5条，2有5条
			},
				...
				if(data.result_code === "0000"){
					...
					if(len === 0){
						...
						data.result.forEach(function(val,index){
							...
							let seviceType = val.seviceType;				//流水单来源（1、押金缴纳2、充值3、押金退还4、借书款5、套餐购买）
							if(seviceType === "1"){
								seviceType = '缴纳';
	...
	jztm (){
		...
		this.z = 1;
		//预先加载10条
		CapitalDetails.queryCapitalList();	//2，加载数据。这个是一上来就加载十条数据。如果没有就显示暂无数据
		// 上次加载的序号
		this.lastIndex = 10;
		$.init();
	},
	publicFun(){
		...
		setTimeout(()=>{
			...
			CapitalDetails.queryCapitalList(this.z,this.itemsPerLoad);
			// 更新最后加载的序号
